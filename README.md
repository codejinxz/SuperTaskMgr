...
- **传感器**：CPU 每核频率与占用率、热区温度、SMART 磁盘健康细项（NVMe 备件%/磨损%/通电时长、SATA 温度）、GPU 引擎级利用率与显存、每适配器网络吞吐、电池、内存/分页池、风扇（诚实标注"需要驱动支持"——本应用不内置内核驱动）。各分组**可勾选显示**；可选接入 **LibreHardwareMonitor 数据源**（默认关：自行运行 LHM 并启用其 Web 服务器，本工具仅读本机 127.0.0.1，实现 HWiNFO 级覆盖且不随包分发驱动）。
