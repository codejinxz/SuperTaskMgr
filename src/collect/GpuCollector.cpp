...
// decided from three independent signals:
...
//   3. Engine counters: a hardware GPU with a WDDM driver exposes \GPU Engine
//      instances carrying its LUID; IddCx display-only adapters never do.
//      Rule 3 is applied conservatively to avoid false positives on idle real
//      GPUs (e.g. a dGPU no process has touched): it only filters a NON-primary
//      adapter while at least one OTHER adapter positively has engine counters,
//      and never when PDH is unavailable or the machine exposes a single
...
struct Adapter {
    ...
    bool software = false;
    bool primary = false;    // first non-software adapter DXGI enumerates
};
...
void GpuCollector::Collect(const std::unordered_map<uint32_t, uint64_t>& createTimeByPid,
        ...
        if (SUCCEEDED(::CreateDXGIFactory1(IID_PPV_ARGS(&factory)))) {
            ...
            for (UINT i = 0; factory->EnumAdapters1(i, &a) != DXGI_ERROR_NOT_FOUND; ++i) {
                ...
                if (SUCCEEDED(a->GetDesc1(&d))) {
                    ...
                    ad.primary = firstNonSoftware && !ad.software;
                    if (!ad.software) firstNonSoftware = false;
    ...
    std::set<uint64_t> engineLuids;  // LUIDs positively present in \GPU Engine
    ...
    if (!pdhFailed_) {
        ...
        if (query_ && PdhCollect(query_)) {
            ...
            if (haveArrays) {
                ...
                // for one sample; those count for presence but not for values.
                for (const PdhArrayItem& it : utils) {
                    uint32_t pid = 0;
                    uint64_t luid = 0;
                    if (!ParseGpuInstance(it.name, &pid, &luid)) continue;
                    engineLuids.insert(luid);
    ...
    const bool anyEngine = !engineLuids.empty();
    const bool singleGpu = [&]() {
        int n = 0;
    ...
    for (const Adapter& a : adapters) {
        ...
        if (a.software) {
            ...
            keep = false;  // signal 2
        } else if (!pdhFailed_ && anyEngine && !singleGpu && !a.primary &&
                   engineLuids.count(a.luid) == 0) {
            keep = false;  // signal 3 (conservative form, see file comment)
        }
